...
int TestCShapDLLMagicNumber() { return 123456; }
    ...
    /*
        ...
        */
        ...
        std::unique_ptr<MeshDecoder> decoder_2 = std::move(std::move(decoder_statusor).value());
        ...
        if (!decode_statusor.ok())
            return -7;
        
        /*
        draco::Status decode_status = decoder_2->Decode(options, tmp_buffer, in_mesh.get());
        if (decode_status.code() != draco::Status::OK) {
        ...
        *tmp_mesh = new DracoToUnityMesh();
        ...
        for (draco::FaceIndex face_id(0); face_id < in_mesh->num_faces(); ++face_id) {
            ...
            memcpy(unity_mesh->indices + face_id.value() * 3, face.data(), sizeof(int) * 3);
        }
        
        //std::unique_ptr<draco::PointCloud> pc;
        //pc = std::move(in_mesh);
        unity_mesh->position = new float[in_mesh->num_points() * 3];
        const auto pos_att = in_mesh->GetNamedAttribute(draco::GeometryAttribute::POSITION);
        std::array<float, 3> value;
        for (draco::PointIndex i(0); i < in_mesh->num_points(); ++i) {
            const draco::AttributeValueIndex val_index = pos_att->mapped_index(i);
            if (!pos_att->ConvertValue<float, 3>(val_index, &value[0]))
                return -8;
            memcpy(unity_mesh->position + i.value() * 3, value.data(), sizeof(float) * 3);
        }
